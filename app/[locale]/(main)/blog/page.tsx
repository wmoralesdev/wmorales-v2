...
export default async function BlogPage({ params, searchParams }: PageProps) {
  ...
  const { posts: filteredPosts, totalCount, filteredCount, availableTags } = searchResults;
  ...
  const featuredPosts = shouldShowFeatured ? filteredPosts.filter((post: any) => post.entry.featured).slice(0, 3) : [];
  ...
  return (
    <div className="py-12 text-center">
      <div className="mx-auto max-w-md rounded-lg border border-gray-800 bg-gray-900/60 p-8 backdrop-blur-xl">
        ...
        {hasFilters && <p className="text-gray-500 text-sm">Total posts available: {totalPosts}</p>}
      </div>
    </div>
  );
}
      ...
      {shouldShowFeatured && featuredPosts.length > 0 && (
        <div className="mt-16">
          <div className="mb-6 flex items-center gap-2">
            ...
            <h2 className="font-semibold text-2xl text-white">Featured Posts</h2>
          </div>
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
