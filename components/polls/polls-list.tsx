...
export function PollsList({ polls }: PollsListProps) {
  ...
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      {polls.map((poll) => (
        <Card className="transition-shadow hover:shadow-lg" key={poll.id}>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-4 text-muted-foreground text-sm">
              ...
              <div className="flex items-center gap-1">
                <span>{poll._count.sessions} participants</span>
              </div>
            </div>
            ...
            <div className="text-muted-foreground text-xs">Created {new Date(poll.createdAt).toLocaleDateString()}</div>
          </CardContent>
        </Card>
      ))}
    </div>
